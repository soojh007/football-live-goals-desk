  ...
  async send(report) {
    ...
    const response = await this.fetchImpl("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: this.from,
        to: this.to,
        subject: `Football totals digest: ${report.date} (${report.candidates.length} picks)`,
  ...
  const cards = report.candidates.map((candidate) => `
    <tr>
      ...
      <td style="padding:14px;border-bottom:1px solid #dfe8e3;text-align:right">
        <div style="font-size:16px;font-weight:800;color:${candidate.side === "OVER_2_5" ? "#087f5b" : "#9a6700"}">${escapeHtml(candidate.label)}</div>
  ...
  <body style="margin:0;background:#f3f7f5;font-family:Arial,sans-serif;color:#102019">
    <div style="max-width:760px;margin:0 auto;padding:24px">
      <div style="background:#0d211a;color:white;padding:22px;border-radius:14px 14px 0 0">
        <div style="font-size:12px;color:#66e0aa;text-transform:uppercase;letter-spacing:1px">Daily match intelligence</div>
        <h1 style="margin:8px 0 4px">Over/Under 2.5 Digest</h1>
        <div style="color:#a9beb5">${escapeHtml(report.date)} · ${report.analyzed} matches analysed · ${report.requestsUsed} API requests</div>
      </div>
